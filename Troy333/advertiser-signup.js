document.addEventListener("DOMContentLoaded", function () {
  ...
  const businessLicenseGroup = document.getElementById(
    "business-license-group"
  );
  ...
  function validateForm() {
    ...
    if (!passwordPattern.test(password)) {
      alert(
        "비밀번호는 영문, 숫자, 특수문자를 포함하여 8~20자리로 입력해주세요."
      );
      return false;
    }
    ...
    const selectedType =
      document.querySelector(".type-card.active").dataset.type;
  ...
  // 입력 필드 실시간 포맷팅
  document
    .getElementById("business-number")
